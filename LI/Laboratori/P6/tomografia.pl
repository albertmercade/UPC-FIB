...
p:-	ejemplo2(RowSums,ColSums),
	...
	declareConstraints(MatrixByRows, MatrixTransposed, RowSums, ColSums, L),
...
declareConstraints(MRows, MCols, RowSums, ColSums, L):-
    L ins 0..1,
    checkSum(MRows,RowSums),
    checkSum(MCols,ColSums),
    label(L).

checkSum([], []):- !.
checkSum([Mi|Mat], [Si|Sums]):- sum(Mi, #=, Si), checkSum(Mat, Sums).
