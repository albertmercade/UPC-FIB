...
writeClauses(MaxComplexTeams):-
    ...
    eachTminSizemaxSize,
    newCT,
    maxThresholdTeams(MaxComplexTeams),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

compatibleWorkers :- worker(W1), worker(W2), W1 \= W2,
    incompatibleWorkers(W1,W2), team(T),
    writeClause( [-wt(W1,T), -wt(W2,T)] ), fail.
compatibleWorkers.

eachWexactlyOneT :- worker(W),
    findall(wt(W,T), team(T), Lits),
    exactly(1,Lits), fail.
eachWexactlyOneT.

eachTminSizemaxSize :- team(T),
    findall(wt(W,T), worker(W), Lits),
    minSize(N), maxSize(X), atLeast(N,Lits), atMost(X,Lits), fail.
eachTminSizemaxSize.

newCT :- worker(W1), worker(W2), W1 \= W2,
    workerScore(W1,S1), workerScore(W2,S2),
    complexThreshold(C),
    S is S1 + S2, C < S,
    team(T),
    writeClause([-wt(W1,T), -wt(W2,T), c(T)]), fail.
newCT.

maxThresholdTeams(MaxComplexTeams) :- findall(c(T), team(T), L),
    atMost(MaxComplexTeams, L), fail.
maxThresholdTeams(_).
...
costOfThisSolution(M,Cost):- findall(T,(team(T),complexTeam(M,T)),L), sort(L,LF), length(LF,Cost), !.

complexTeam(M,T) :- member(wt(W1,T),M), member(wt(W2,T),M), W1 \= W2,
    workerScore(W1,S1), workerScore(W2,S2),
    S is S1 + S2, complexThreshold(TH), S > TH, !.
