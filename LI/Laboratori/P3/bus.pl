...
satVariable( trip1(B,D1,C1,C2) ) :- bus(B), day(D1), city(C1), city(C2).
satVariable( trip2(B,D1,C1,C2,C3) ) :- bus(B), day(D1), city(C1), city(C2), city(C3).
...
writeClauses:-
    ...
    busDiferentCity,
    exactlyOneCperBperD,
    atLeastOneRoutePerCPair,
    maxDistConsecDperB,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

trip1BDC:- bus(B), day(D1), trip(C1-C2), consecutiveDays(D1, D2),
    writeClause([-trip1(B,D1,C1,C2),bdc(B,D1,C1)]),
    writeClause([-trip1(B,D1,C1,C2),bdc(B,D2,C2)]),
    writeClause([trip1(B,D1,C1,C2),-bdc(B,D1,C1),-bdc(B,D2,C2)]),
    fail.
trip1BDC.

trip2BDC:- bus(B), day(D1), trip(C1-C2), trip(C2-C3), consecutiveDays(D1,D2,_),
    writeClause([-trip2(B,D1,C1,C2,C3),trip1(B,D1,C1,C2)]),
    writeClause([-trip2(B,D1,C1,C2,C3),trip1(B,D2,C2,C3)]),
    writeClause([trip2(B,D1,C1,C2,C3),-trip1(B,D1,C1,C2),-trip1(B,D2,C2,C3)]),
    fail.
trip2BDC.

exactlyOneBperCperD:- city(C), day(D), findall(bdc(B,D,C), bus(B), Lits),
    exactly(1, Lits), fail.
exactlyOneBperCperD.

busDiferentCity:- bus(B), day(D1), city(C), notLastDay(D1),
    consecutiveDays(D1, D2), writeClause([-bdc(B,D1,C),-bdc(B,D2,C)]), fail.
busDiferentCity.

exactlyOneCperBperD:- bus(B), day(D), findall(bdc(B,D,C), city(C), Lits),
    exactly(1, Lits), fail.
exactlyOneCperBperD.

atLeastOneRoutePerCPair:- trip(C1-C2), findall(trip1(B,D1,C1,C2), (bus(B),
    consecutiveDays(D1,_)), Lits), atLeast(1, Lits), fail.
atLeastOneRoutePerCPair.

maxDistConsecDperB:- maxDist(MX), findall(trip2(B,D1,C1,C2,C3), (bus(B),
    consecutiveDays(D1, _, _), trip(C1-C2), trip(C2-C3), dist(C1,C2,Dist1),
    dist(C2,C3,Dist2), Dist1 + Dist2 > MX), Lits), atMost(0, Lits), fail.
maxDistConsecDperB.
