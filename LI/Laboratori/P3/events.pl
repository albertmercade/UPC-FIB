...
possibleEvents :- event(E), event(E,ML,DL), member(M,ML), member(D,DL), writeClause([-em(E,M), -ed(E,D), md(M,D)]), fail.
possibleEvents.

eachEexactlyOneM :- event(E), findall( em(E,M), (eventModerators(E,ML), member(M,ML)), Lits), exactly(1,Lits), fail.
eachEexactlyOneM.

eachEexactlyOneD :- event(E), findall( ed(E,D), (eventDays(E,DL), member(D,DL)), Lits), exactly(1,Lits), fail.
eachEexactlyOneD.

eachDmaxEvents :- day(D), findall(ed(E,D), (event(E), eventDays(E,DL), member(D,DL)), Lits), maxEventsPerDay(X), atMost(X,Lits), fail.
eachDmaxEvents.

eachMmaxDays :- moderator(M), findall(md(M,D), day(D), Lits), maxDaysPerModerator(X), atMost(X,Lits), fail.
eachMmaxDays.
