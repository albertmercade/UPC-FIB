...
incompatibleWorkers(W1,W2):- workerScore(W1, S1), workerScore(W2,S2), maxScore(S), S1 + S2 > S.
...
compatibleWorkers :- worker(W1), worker(W2), W1 \= W2, incompatibleWorkers(W1,W2), team(T), writeClause( [-wt(W1,T), -wt(W2,T)] ), fail.
compatibleWorkers.

eachWexactlyOneT :- worker(W), findall(wt(W,T), team(T), Lits), exactly(1,Lits), fail.
eachWexactlyOneT.

eachTminSizemaxSize :- team(T), findall(wt(W,T), worker(W), Lits), minSize(N), maxSize(X), atLeast(N,Lits), atMost(X,Lits), fail.
eachTminSizemaxSize.
