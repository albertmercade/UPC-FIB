...
void MyGLWidget::capsaContenidoraEscena() {
    centreEsc = glm::vec3((Pmin[0]+Pmax[0])/2,(Pmin[1]+Pmax[1])/2,(Pmin[2]+Pmax[2])/2);
...
{
    if(llumPilota) posFocus = glm::vec3(posPilota[0], 6.5, posPilota[2]);
    else posFocus = glm::vec3(0, 0, 0);

    glUniform3fv(posFocusLoc, 1, &posFocus[0]);
    glUniform1i(llumPilotaLoc, llumPilota);
}

void MyGLWidget::focusPosicio(int pos) {
    makeCurrent();
    if(pos == 0) llumPilota = false;
    else llumPilota = true;
    posicionaFocus();
    update();
}
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)  // Cal modificar aquesta funció...
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_F: {
      ...
      llumPilota = !llumPilota;
      if(llumPilota) emit setPosicio(1);
      else emit setPosicio(0);
      posicionaFocus();
      break;
    }
...
void MyGLWidget::carregaShaders()  // mètode que et cal modificar...
{
  ...
  llumPilotaLoc = glGetUniformLocation(program->programId(), "llumPilota");
}
