...
void MyGLWidget::iniEscena ()   // Cal modificar aquest mètode...
{
  ...
  centreEsc = glm::vec3 (10.0, 3.0, 10.0);
  radiEsc = sqrt(pow(20.0,2.0) + pow(6.0,2.0) + pow(20.0,2.0))/2.0;
...
void MyGLWidget::resizeGL (int w, int h)   // Cal modificar aquest mètode...
{
  ...
  alt = h;
  ra = float(w)/float(h);
  if(ra <= 1.0) fov = 2.0 * atan(tan(asin(0.5))/ra);
  projectTransform();
}
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  glm::mat4 TG (1.0f); // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(2.0, 0.0, 2.0));
  TG = glm::rotate(TG,float(M_PI/4.0),glm::vec3(0,1,0));
  TG = scale(TG, glm::vec3(6.0*escalaPatr,6.0*escalaPatr,6.0*escalaPatr));
  TG = translate(TG,-centreBasePatr);
  glUniformMatrix4fv(transLoc, 1, GL_FALSE, &TG[0][0]);
}
...
void MyGLWidget::projectTransform ()
{
  ...
  if(camera3) Proj = glm::perspective(fov, ra, zn, zf);
