...
public class BicingSuccessorFunction2 implements SuccessorFunction {
	...
	public List getSuccessors(Object state) {
		...
		aux = board.estacionesNoCargadas();
		for(int i=0; i<currentCity.getSizeFlota();++i) {
			for(int j=0; j<aux.size(); ++j) {
				try {
					successor = board.unloadFurgoToRound2(currentCity.getFlota().getFurgoneta(i), aux.get(j));
				} catch (Exception e) {
					// TODO: handle exception
				}
				if(successor != null) {
					Estacion est = aux.get(j);
					successors.add(new Successor("unloadRound2->["+i+",("+est.getCoordX()+","+est.getCoordY()+")]| ingreso = "+successor.getIngresos()+" | beneficio = "+(Math.round(successor.getBeneficios()*100.0)/100.0)+" | distancia = "+successor.getDistanciaAgregada(), successor));
				}
			}
		}
